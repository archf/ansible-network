...
iptables $use_iptables_lock -I INPUT -i ${LXC_BRIDGE} -p udp --dport 68 -j ACCEPT
iptables $use_iptables_lock -I INPUT -i ${LXC_BRIDGE} -p tcp --dport 68 -j ACCEPT
```

##  ipv6 forwarding on bridge
...
echo 1 > /proc/sys/net/ipv6/conf/${LXC_BRIDGE}/accept_ra_from_local
